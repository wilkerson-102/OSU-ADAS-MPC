...
reld_sym = MX.sym('reld_sym',1);
relv_sym = MX.sym('relv_sym',1);
af_sym = MX.sym('af_sym',1);
vf_sym = MX.sym('vs_sym',1);
afref_sym = MX.sym('afref_sym',1);
ap_sym = MX.sym('ap_sym',1);

% Converting from distance error from desired
r = 1;
tauh = 1;
vf_high = 25;
vf_low = 10;
...
KL = 1.0;
do = 10;
ddes = r*(vf_sym-vfmean)+tauh*vf_sym+do;
deltad = reld_sym-ddes;

% Building state
x_sym = [deltad ; relv_sym ; af_sym];

% update MPC parameters
% obj.opti.set_value(obj.X{1},x_sym)
% obj.opti.set_value(obj.Vf{1},vf)
% obj.opti.set_value(obj.AfRef{1},afref)
% obj.opti.set_value(obj.Ap,ap)

% Solve problem
sol_sym = obj.opti.solve();

% Get input value
u = sol.value(sol.value(obj.U{1}));
...
function_name = 'ACC';
ACC = Function(function_name,{[reld_sym ; relv_sym ; af_sym ; vf_sym ; afref_sym ; ap_sym]},...
    {sol_sym.value(sol_sym.value(obj.U{1}))});
